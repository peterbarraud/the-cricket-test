...
def makebattercsv(info : CricketInfo):
    ...
    for match_file in match_file_generator():
        ...
        for innings_number, innings_card in enumerate(match_cards):
            ...
            batter_title_cell : Tag = innings_card.find(class_=info.BatterTitleCell)
            batter_row : Tag = batter_title_cell.parent()
            name = batter_title_cell.get_text()
            print(batter_name_row.parent())
            break
